# train_irritation.py
# =========================
# LSTM model với đầu vào: MACCS + Physchem
...
train_df = pd.read_csv("x_train/x_train_irritation.csv")
...
X = np.array(X).reshape(len(X), 1, -1)  # Reshape để vào LSTM
y = np.array(y)

# Xây dựng mô hình LSTM
model = Sequential()
model.add(LSTM(64, return_sequences=True, input_shape=(1, X.shape[2])))
model.add(LSTM(32))  # Không cần return_sequences=True ở tầng cuối
...
model.save("models/irritation_model.keras")
